...
class Agent:
    ...
    def sense(self, env):
        ...
        Updates:
            self.channel_inputs: a sensors x (max channels + 1) np array.
    ...
    def act(self, env):
        ...
        Arguments:
            env: a np array of size x size x channels + 1.
                Where [:,:,-1] stores the maze structure.
            channel_inputs: a sensors x max channels np array, generated by sense.
        Updates:
        ...
        output += np.random.rand(len(output)) / 1000
        ...
        # output[np.where(1 - env[new_rc[0], new_rc[1], -1])[0].astype(int)] = -np.inf

        # Choose action
        action = np.argmax(output)

        # # Probabalistic
        # sm_output = np.exp(output - np.max(output)) # softmax
        # sm_output /= sm_output.sum(axis=0) # softmax
        # action = np.random.choice(range(4), p=sm_output)
