...
class QLearnerAgentContinuous:
    ...
    """
    def __init__(self, pk_hw, channels, location, sensor_noise_scale, n_steps, n_features, cost_per_step, cost_per_collision, alpha, epsilon, gamma):
        self.pk_hw = pk_hw
        ...
        self.theta = np.zeros((n_features, 2))
        self.max_velocity = 2
        
    def reset(self):
        """Resets the agent to its initial location and angle."""
        self.velocity = 0
    ...
    def sense_features(self, location):
        ...
        features[0] = self.env[np.floor(location[0]).astype(int) - 1, np.floor(location[1]).astype(int), 0]         # Sensory information up
        features[1] = self.env[np.floor(location[0]).astype(int) + 1, np.floor(location[1]).astype(int), 0]         # Sensory information down
        features[2] = self.env[np.floor(location[0]).astype(int), np.floor(location[1]).astype(int) + 1, 0]         # Sensory information right
        features[3] = self.env[np.floor(location[0]).astype(int), np.floor(location[1]).astype(int) - 1, 0]         # Sensory information left
        features[4] = self.env[np.floor(location[0]).astype(int) - 1, np.floor(location[1]).astype(int) + 1, 0]     # Sensory information up-right
        features[5] = self.env[np.floor(location[0]).astype(int) - 1, np.floor(location[1]).astype(int) - 1, 0]     # Sensory information up-left
        features[6] = self.env[np.floor(location[0]).astype(int) + 1, np.floor(location[1]).astype(int) + 1, 0]     # Sensory information down-right
        features[7] = self.env[np.floor(location[0]).astype(int) + 1, np.floor(location[1]).astype(int) - 1, 0]     # Sensory information down-left
        
        scaled_distance, scaled_angle = self.closest_prey_features(self.prey_locations, location)[1:]
        ...
        offsets[:, 1] = (self.prey_velocities * self.last_seen).astype(float)
        predicted_next_prey_locations = self.prey_locations + offsets
        pred_scaled_distance, pred_scaled_angle = self.closest_prey_features(predicted_next_prey_locations, location)[1:]

        features[10] = pred_scaled_distance     # Predicted distance
        features[11] = pred_scaled_angle        # Predicted angle

        # noisy_features = features + np.random.normal(0, self.sensor_noise_scale, features.shape)
    ...
    def closest_prey_features(self, prey_locations, location):
        ...
        scaled_angle = np.arctan2(delta[1], delta[0]) / np.pi
    ...
    def policy(self, action):
        ...
        delta_x = velocity * np.cos(angle)
        delta_y = velocity * np.sin(angle)
        next_location = self.location + np.array([delta_x, delta_y])
        next_distance = self.closest_prey_features(self.prey_locations, next_location)[1]
        delta_distance = next_distance - self.closest_prey_features(self.prey_locations, self.location)[1]
        
        if self.env[np.floor(next_location[0]).astype(int), np.floor(next_location[1]).astype(int), -1] == 1:
            ...
            if velocity != self.last_velocity or angle != self.last_angle:
                acceleration = (velocity - self.last_velocity) * (angle - self.last_angle)
                reward -= (200 * abs(acceleration))
    ...
    def act(self, env, prey_locations, prey_velocities, visible=True, training=False):
        """
        ...
        """
        self.env = env
        self.prey_velocities = prey_velocities
        ...
        if visible:
            ...
            offsets[:, 1] = (prey_velocities * self.last_seen).astype(float)
            self.prey_locations = self.last_seen_prey_locations + offsets

        velocity = self.random_continuous_action() if np.random.rand() < self.epsilon else self.optimal_action(self.location)
        next_location, reward = self.policy(velocity)
        self.location = next_location

        if training:
            next_velocity = self.optimal_action(next_location)
            self.learn(self.location, next_location, reward)
    ...
    def random_continuous_action(self):
        """
        Generates a random continuous action, e.g., random velocity and angle.
        """
        velocity = np.random.uniform(0, self.max_velocity)  # Random velocity
        angle = np.random.uniform(-np.pi, np.pi)  # Random angle (in radians)
        return np.array([velocity, angle])

    def optimal_action(self, location):
        """
        Determines the optimal action in a continuous space using the learned Q-values.
        """
        q_values = np.dot(self.sense_features(location), self.theta)

        # scale instead of clip
        # find way to choose the optimal angle and velocity

        optimal_velocity = np.clip(q_values[0], 0, self.max_velocity)
        optimal_angle = np.clip(q_values[1], -np.pi, np.pi)

        return np.array([optimal_velocity, optimal_angle])

    def learn(self, location, next_location, reward):
        """
        ...
        """
        # Q = self.Q_value(location)
        # Q_next =  self.Q_value(next_location)
        # TD_error = reward + (self.gamma * Q_next) - Q
        # self.theta += self.alpha * TD_error * self.sense_features(location) 

        # Check the dimensions of all of the q-values           
        Q_velocity = np.dot(self.sense_features(location), self.theta[:, 0])
        Q_next_velocity = np.dot(self.sense_features(next_location), self.theta[:, 0])
        
        Q_angle = np.dot(self.sense_features(location), self.theta[:, 1])
        Q_next_angle = np.dot(self.sense_features(next_location), self.theta[:, 1])

        # Calculate the TD error for both velocity and angle
        TD_error_velocity = reward + (self.gamma * Q_next_velocity) - Q_velocity
        TD_error_angle = reward + (self.gamma * Q_next_angle) - Q_angle

        # Update the weight parameters theta using the TD error for velocity and angle
        self.theta[:, 0] += self.alpha * TD_error_velocity * self.sense_features(location)
        self.theta[:, 1] += self.alpha * TD_error_angle * self.sense_features(location)
    ...
    def produce_plots(self, training_lengths, first_5_last_5, percentage_captured, animate, trials, trial_lengths):
        ...
        """
        self.plotter = GridPlotterContinuous(self)
        if training_lengths:
