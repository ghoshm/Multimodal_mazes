...
class QNetwork(nn.Module):
    def __init__(self, input_dim, output_dim, hidden_dim, flags=None):
        super(QNetwork, self).__init__()

        self.f_input_hidden = nn.Linear(input_dim, hidden_dim)
        self.f_hidden_output = nn.Linear(hidden_dim, output_dim)

        torch.nn.init.xavier_uniform_(self.f_input_hidden.weight, gain=1.0, generator=None)
        torch.nn.init.xavier_uniform_(self.f_hidden_output.weight, gain=1.0, generator=None)

        # Lateral connections
        self.lateral_input = flags.get('lateral_input', False)
        self.lateral_hidden = flags.get('lateral_hidden', False)
        self.lateral_output = flags.get('lateral_output', False)
        if self.lateral_input:
            self.l_input = nn.Linear(input_dim, input_dim)
        if self.lateral_hidden:
            self.l_hidden = nn.Linear(hidden_dim, hidden_dim)
        if self.lateral_output:
            self.l_output = nn.Linear(output_dim, output_dim)
        
        # Skip connections
        self.skip_input_output = flags.get('skip_input_output', False)
        self.skip_output_input = flags.get('skip_output_input', False)
        if self.skip_input_output:
            self.f_skip_input_output = nn.Linear(input_dim, output_dim)
        if self.skip_output_input:
            self.b_skip_output_input = nn.Linear(output_dim, input_dim)
        
        # Backward connections
        self.backward_output_hidden = flags.get('backward_output_hidden', False)
        self.backward_hidden_input = flags.get('backward_hidden_input', False)
        if self.backward_output_hidden:
            self.b_output_hidden = nn.Linear(output_dim, hidden_dim)
        if self.backward_hidden_input:
            self.b_hidden_input = nn.Linear(hidden_dim, input_dim)
    ...
    def forward(self, x):
        ...
        # Input lateral
        if self.lateral_input:
            x += torch.relu(self.l_input(x))

        # Forward pass hidden
        hidden_output = torch.relu(self.f_input_hidden(x))
        
        # Hidden lateral
        if self.lateral_hidden:
            hidden_output += torch.relu(self.l_hidden(hidden_output))

        # Backward hidden to input
        if self.backward_hidden_input:
            input_from_hidden = torch.relu(self.b_hidden_input(hidden_output))
            x += input_from_hidden

        # Forward pass output
        output = self.f_hidden_output(hidden_output)

        # Output lateral
        if self.lateral_output:
            output += torch.relu(self.l_output(x))

        # Skip input to output
        if self.skip_input_output:
            output += torch.relu(self.f_skip_input_output(input_original))

        # Skip output to input
        if self.skip_output_input:
            input_from_output = torch.relu(self.b_skip_output_input(output))
            x += input_from_output

        # Backward output to hidden
        if self.backward_output_hidden:
            hidden_from_output = torch.relu(self.b_output_hidden(output))
            hidden_output += hidden_from_output
    ...
    def reset_parameters(self):
        self.f_input_hidden.reset_parameters()
        self.f_hidden_output.reset_parameters()

        # Reset lateral, skip, and backward connections if they exist
        if self.lateral_input:
            self.l_input.reset_parameters()
        if self.lateral_hidden:
            self.l_hidden.reset_parameters()
        if self.lateral_output:
            self.l_output.reset_parameters()

        if self.skip_input_output:
            self.f_skip_input_output.reset_parameters()
        if self.skip_output_input:
            self.b_skip_output_input.reset_parameters()

        if self.backward_output_hidden:
            self.b_output_hidden.reset_parameters()
        if self.backward_hidden_input:
            self.b_hidden_input.reset_parameters()
...
class DQNLearnerAgent:
    def __init__(self, input_dim, output_dim, hidden_dim, flags, lr, gamma, epsilon, batch_size=32, pk_hw=None, channels=None, actions=None, location=None, sensor_noise_scale=None, n_steps=None, n_features=None, cost_per_step=None, cost_per_collision=None, max_distance=21):
        ...
        self.q_network = QNetwork(input_dim, output_dim, hidden_dim, flags)
        self.target_network = QNetwork(input_dim, output_dim, hidden_dim, flags)
        self.optimizer = optim.Adam(self.q_network.parameters(), lr=lr)
        self.scheduler = optim.lr_scheduler.ReduceLROnPlateau(self.optimizer, factor=0.1, patience=10, min_lr=0.001)
    ...
    def reset(self):
        ...
        self.location = np.copy(np.array([self.pk_hw + (20 // 2), self.pk_hw + (21 // 2)]))
    ...
    def sense_features(self, location):
        ...
        features[4] = self.env[location[0] - 1, location[1] + 1, 0]
        features[5] = self.env[location[0] - 1, location[1] - 1, 0]
        features[6] = self.env[location[0] + 1, location[1] + 1, 0]
        features[7] = self.env[location[0] + 1, location[1] - 1, 0]

        scaled_distance, scaled_angle = self.closest_prey_features(self.prey_locations, location)

        features[8] = scaled_distance
        features[9] = scaled_angle
        ...
        pred_scaled_distance, pred_scaled_angle = self.closest_prey_features(predicted_next_prey_locations, location)

        features[10] = pred_scaled_distance
        features[11] = pred_scaled_angle
    ...
    def policy(self, action):
        ...
        next_distance = self.closest_prey_features(self.prey_locations, next_location)[0]
        distance_delta = next_distance - self.closest_prey_features(self.prey_locations, self.location)[0]
        
        if self.env[next_location[0], next_location[1], -1] == 1:
            if next_location in self.prey_locations:
                reward = 1000
            else: 
                reward =  -(self.max_distance * distance_delta * 100)
    ...
    def act(self, env, prey_locations, prey_directions, prey_pm, visible=True):
        ...
        features = self.sense_features(self.location)
    ...
    def training_act(self, env, prey_locations, prey_directions, prey_pm, visible=True):
        ...
        action = self.epsilon_greedy_policy(self.epsilon, state_tensor)
        next_location, reward = self.policy(action)
        next_features = self.sense_features(next_location)
        
        self.replay_buffer.add(features, action, reward, next_features)
    ...
    def epsilon_greedy_policy(self, epsilon, state_tensor):
    ...
    def minimum_trial_length(self, location, prey_locations):
        ...
        while len(prey_locations) > 0:
            nearest_prey = min(prey_locations, key=lambda prey: np.linalg.norm(location - np.array(prey)))
            delta = location - nearest_prey
